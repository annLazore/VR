...
let rotationMatrix = m4.identity(); // поточна матриця обертання з сенсора
let sensorSocket;
let quaternion = [0, 0, 0, 1]; // w, x, y, z — стиль OpenGL
...
function draw() {
    ...
    const modelView = quaternionToMatrix(quaternion);
...
function connectToSensorServer() {
    const WS_URL = "ws://192.168.0.103:8080/sensor/connect?type=android.sensor.game_rotation_vector";
    sensorSocket = new WebSocket(WS_URL);

    sensorSocket.onopen = () => {
        console.log("✅ Підключено до Sensor Server через direct sensor path");
    };
    ...
    sensorSocket.onmessage = (event) => {
        ...
        try {
            ...
            if (Array.isArray(data.values) && data.values.length >= 4) {
                const [x, y, z, w] = data.values;
                quaternion = [w, x, y, z]; // порядок для OpenGL
                //console.log("✅ Updated quaternion:", quaternion);
            }
        } catch (e) {
            console.error("❌ Error parsing incoming data:", e, event.data);
        }
    };
